...
if __name__=="__main__":
    ...
    baskets = input_file.combineByKey(lambda x: [x], lambda i, n: mergeVal(i, n, "append"), lambda i, n: mergeVal(i, n, "extend")).map(lambda x: (x[0], set(x[1]))).filter(lambda x: len(x[1]) > threshold).persist()

    partitions = baskets.count()
    Candidates = baskets.mapPartitions(lambda x: MRP1(x, support, partitions)).distinct().collect()
    ...
    with open(output_file, "w") as f:
        ...
        for d in candict:
            fwriter = ""
            candict[d] = sorted(tuple(sorted(x)) for x in candict[d])
            for x in candict[d]:
                if len(tuple(x)) == 1:
